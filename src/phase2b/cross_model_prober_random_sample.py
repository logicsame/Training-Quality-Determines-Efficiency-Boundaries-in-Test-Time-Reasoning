...
class CrossModelProber:
    ...
    def probe_sample(self, sample: Dict, task_type: str, condition: str = "control") -> List[ModelResponse]:
        """
        ...
        prompt = PromptFormatter.format(question, task_type, condition)
        ...
        for model_name, manager in self.model_managers.items():
            try:
                ...
                # length_only condition: same prompt as control but hard cap
                # at generation level — isolates token budget from prompt framing
                if condition == "length_only":
                    LENGTH_ONLY_LIMITS = {
                        'math': 75,        # ~50 words
                        'reasoning': 20,   # yes/no only
                        'commonsense': 30, # single letter + minimal context
                    }
                    max_tokens = LENGTH_ONLY_LIMITS.get(task_type, 75)
