...
def load_gpqa(num_samples: int = 50, split: str = "train") -> Tuple[List[Dict], str]:
    """Load GPQA dataset - PhD-level science questions"""
    print(f"Loading GPQA ({split} split, {num_samples} samples)...")
    ...
    for idx, item in enumerate(dataset.select(range(min(num_samples, len(dataset))))):
        ...
        # Shuffle so correct answer lands randomly at A/B/C/D
        random.shuffle(choices)
        
        # Find where correct answer landed after shuffle
        correct_idx = choices.index(item['Correct Answer'])
        answer_letter = chr(65 + correct_idx).lower()  # 'a', 'b', 'c', or 'd'
    ...
    print(f"✅ Loaded {len(samples)} samples from GPQA")
    return samples, "commonsense"  


def load_bbh(num_samples: int = 50, split: str = "test", task: str = "logical_deduction_three_objects") -> Tuple[List[Dict], str]:
    """Load BBH dataset - Big Bench Hard (challenging reasoning)"""
    print(f" Loading BBH-{task} ({split} split, {num_samples} samples)...")
    
    # Only these tasks have clear multiple choice format
    SUPPORTED_TASKS = [
    ...
    if task not in SUPPORTED_TASKS:
        print(f" Warning: Task '{task}' may not be multiple choice format")
    
    dataset = load_dataset("lukaemon/bbh", task, split=split)
    
    samples = []
    
    for idx, item in enumerate(dataset.select(range(min(num_samples, len(dataset))))):
        question = item['input']
        answer = item['target']  # Format: "(A)" or "(B)" etc.
        
        # Strip parentheses and convert to lowercase
        answer_clean = answer.strip().strip('()').lower()
        
        samples.append({
            'id': idx,
            'question': question,
            'ground_truth': answer_clean,  # Now: 'a', 'b', 'c'
            'dataset': f'bbh_{task}'
        })
    
    print(f"Loaded {len(samples)} samples from BBH-{task}")
    return samples, "commonsense"
